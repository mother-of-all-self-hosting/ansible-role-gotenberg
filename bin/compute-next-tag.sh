...
#
# Tags look like `v<Gotenberg version>-<release>`, which is what this repository
# has always published (v8.31.0-0 ... v8.36.0-0):
#
# - if defaults/main.yml points at a Gotenberg version that has never been
#   released, the release counter restarts at 0 (`v8.37.0-0`)
# - otherwise the counter is incremented (`v8.37.0-1`), but only if something
...
# The commit-message approach this replaced could only ever release a Renovate
# bump: a bugfix in the role sat untagged until the next version bump came along
# and carried it out silently.
...
# Anchored on `gotenberg_version:` so that neither the Renovate annotation above
# it nor `gotenberg_container_image_tag`, which is derived from it, can be
# mistaken for it.
version="$(sed -nE 's|^gotenberg_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Gotenberg version from $defaults_path"
	exit 1
fi

# Gotenberg's own version is carried without a leading `v` (the `v` lives only
# in the tags of this repository), but tolerate one so that a future change of
